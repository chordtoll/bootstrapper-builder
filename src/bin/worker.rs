...
use bootstrapper::{
    ...
    env_substitute,
    network::worker::{
        read_deps, read_envs, read_overlays, read_recipe, read_sources, write_archive,
        StatusUpdate, WorkerStatus,
    },
    recipe::{HydratedRecipeVersion, RecipeBuildStep, SourceContents},
    sanitize_path, WORKER_PORT,
};
...
async fn main() {
    ...
    let mut stream = TcpStream::connect(("127.0.0.1", WORKER_PORT))
        .await
        .unwrap();

    loop {
        stream
            .write_u8(WorkerStatus::ReadyForWork as u8)
            .await
            .unwrap();
        ...
        let recipe = read_recipe(&mut stream).await;

        let source_data = read_sources(&mut stream).await;

        let dep_data = read_deps(&mut stream).await;

        let overlay_data = read_overlays(&mut stream).await;

        let env_data = read_envs(&mut stream).await;
        ...
        write_archive(&mut stream, &hash, &archive).await;
        info!("{}", hash)
    }
}
...
fn build(
    ...
    for (source, source_directive) in recipe.source {
        let (_, data) = source_data
            .get(&source_directive.sha)
            .unwrap_or_else(|| panic!("Missing source {}", source_directive.sha));
        assert!(source.chmod.is_none());
        if let Some(extract) = source.extract {
            if source_directive.url.ends_with(".zip") {
                ...
                todo!("{}", source_directive.url);
            }
        }
        if let Some(noextract) = source.noextract {
    ...
    for dep in recipe.deps {
        let data = dep_data
            .get(&dep.hash.0)
            .unwrap_or_else(|| panic!("Missing dep {:?}", dep));
        tar::Archive::new(std::io::Cursor::new(data)).filter_extract(
            work_dir.path(),
            source_extract_filter(
                &dep.from.map(PathBuf::from).unwrap_or(PathBuf::new()),
                &dep.to.map(PathBuf::from).unwrap_or(PathBuf::new()),
                &None,
                false,
            ),
        )
